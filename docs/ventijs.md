<!--
Provenance header, added by ventijs. Everything below this block is the upstream
file, byte for byte, and is not maintained by hand.

Source: the `ws` project API reference (upstream `doc/ws.md`), copyright Einar
Otto Stangvik, Arnout Kazemier, and contributors, MIT-licensed. Vendored here as
the compatibility contract for `ws` 8.21.3 and `@types/ws` 8.18.1, the versions
ventijs targets. Recording: THIRD_PARTY_NOTICES.md.

How to read this file: it is a reference, not a feature list. It records what
`ws` does, so that ventijs has one authority to compare against. It says nothing
about what ventijs implements. For that, read COMPATIBILITY.md and
docs/compliance.md, which map each item below to a module and a status.

This file is not kept in sync by hand. It is replaced wholesale when the pinned
`ws` version changes, and no local edit to the body survives that replacement.
ventijs is not affiliated with the `ws` project.
-->
