...
Pre-alpha. This branch currently ships the package scaffolding, the TypeScript
toolchain configuration (`pnpm`, `tsdown`, `oxlint`, `oxfmt`, `vitest`), the
native build environment (`flake.nix`, Zig 0.16.0), and this documentation set.
The N-API binding, the `ws` compatibility layer, and the conformance suite are
implemented in subsequent changes. No npm release exists yet; do not deploy this
in production.
